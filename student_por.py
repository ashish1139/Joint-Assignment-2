...
for column in categorical_columns:
    categorical_stats[column] = df2[column].value_counts()
...
detailed_stats = df2.describe(include='all')
...
sns.histplot(df2['G3'], kde=True)
plt.title('Distribution of Final Grades (G3)')
plt.show()

# Pairplot to see relationships
sns.pairplot(df2[['G3', 'G1', 'G2', 'age', 'studytime', 'failures', 'absences']])
plt.show()
...
subset_df2 = df2[columns_of_interest]

# Compute the correlation matrix for the selected columns
corr_matrix2 = subset_df2.corr()
